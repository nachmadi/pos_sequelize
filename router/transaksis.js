      ...
      res.send('minimum belanja Rp. 1000');
        ...
        if((deposito-total)<=1000){
          ...
          .then(trx=>{
            ...
            models.Users.findOne({where:{id:req.session.login.userId}})
            .then(user=>{
              let oldDeposit=user.deposito;
              let newDeposito=oldDeposit-req.body.total;
              objDep={deposito:newDeposito}
              models.Users.update(objDep,{where:{id:req.session.login.userId}})
              .then(dep=>{
                models.Item.findAll()
                .then(items=>{
                  let c=0;
