...
router.get('/',(req, res)=>{
    res.render('report',{role:req.session.login.role});
});
...
router.get('/lu',(req, res)=>{
  ...
  .then(hasil=>{
//       res.send(hasil);
       res.render('lap_customer',{reportCus:hasil,role:req.session.login.role});
  })
   .catch(error=> {
      res.send(error);
   });
...
router.get('/lb',(req, res)=>{
  ...
  .then(hasil=>{
       res.render('lap_barang',{reportBrg:hasil,role:req.session.login.role});
  ...
  if(req.body.cmb_JnsTrans==1){
    ...
    .then(hasil=>{
     ...
     .catch(error=> {
      ...
      .then(hasil=>{
          res.render('lap_transaksi',{reportTrans:hasil,role:req.session.login.role});
      })
      .catch(error=> {
         res.send(error);
      });
